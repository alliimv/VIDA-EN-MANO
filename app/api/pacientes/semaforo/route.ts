...
import { determinarEstadoPaciente, ESTADO_TEXTO } from '@/lib/utils';
...
export async function GET() {
  try {
    ...
    const pacientes = rows.map((r: any) => {
      ...
      const estado = determinarEstadoPaciente(
        r.temperatura_c,
        r.ritmo_cardiaco,
        r.esta_puesta
      );
    ...
    console.error('Error al obtener datos del semáforo:', error);
